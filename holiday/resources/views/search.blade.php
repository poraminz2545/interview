                        ...
                        <td>
                            <form action="{{ route('update_status') }}" method="POST" id="editForm-{{ $d->id }}">
                                ...
                                <select name="status" class="form-select">
                                    <option value="รอพิจารณา" {{ $d->status == 'รอพิจารณา' ? 'selected' : '' }}>รอพิจารณา
                                    </option>
                                    <option value="อนุมัติ" {{ $d->status == 'อนุมัติ' ? 'selected' : '' }}>อนุมัติ
                                    </option>
                                    <option value="ไม่อนุมัติ" {{ $d->status == 'ไม่อนุมัติ' ? 'selected' : '' }}>
                                        ไม่อนุมัติ</option>
                                </select>
                                <button type="button" class="btn btn-primary mt-2"
                                    onclick="confirmEdit('{{ $d->id }}')">Edit Status</button>
                            </form>
                        </td>
                        <td>
